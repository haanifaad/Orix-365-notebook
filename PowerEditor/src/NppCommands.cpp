...
void Notepad_plus::command(int id)
{
	switch (id)
	{
		...
		{
			NppParameters& nppParams = NppParameters::getInstance();
			nppParams.getNppGUI()._darkmode._isEnabled = false;
			NppDarkMode::setWindowsMode(false);
			::SendMessage(_pPublicInterface->getHSelf(), NPPM_INTERNAL_REFRESHDARKMODE, static_cast<WPARAM>(true), 0);
			break;
		}
		...
		{
			NppParameters& nppParams = NppParameters::getInstance();
			nppParams.getNppGUI()._darkmode._isEnabled = true;
			NppDarkMode::setWindowsMode(false);
			::SendMessage(_pPublicInterface->getHSelf(), NPPM_INTERNAL_REFRESHDARKMODE, static_cast<WPARAM>(true), 0);
			break;
		}
